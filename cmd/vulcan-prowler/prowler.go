...
func runProwler(ctx context.Context, logger *logrus.Entry, cfg aws.Credentials, region string, groups []string) (*prowlerReport, error) {
	...
	// TODO: Pass env to toe command.Execute function when it is implemented by vulcan-check-sdk
	// env := []string{
	...
	// }
	// output, status, err := command.ExecuteEnvs(ctx, logger, env, prowlerCmd, params...)
	os.Setenv(envKeyID, cfg.AccessKeyID)
	os.Setenv(envKeySecret, cfg.SecretAccessKey)
	os.Setenv(envToken, cfg.SessionToken)
